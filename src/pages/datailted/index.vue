  ...
  <div class="dd_fenlei page">
    <Header title="详情" icon />
      <section class="section">
        ...
        <div class="introduce">
          <p>{{list.name}} {{list.author_name}}</p>
          <div
            class="jianjie"
          >{{list.subtitle}}</div>
          <a href="#">
          ...
          <div class="new_style">
            <div class="new_style-one">
              <div class="new_style-left">
                <b>￥</b>
                <span>{{Math.floor(list.price/3).toFixed(2)}}</span>
                <i>((3.90折))</i>
                <u>降价通知</u>
              </div>
...
<script>
import {detailtedApi} from "@api/datailted"
export default {
  name: "dateilted",
    data(){
        return{
    ...
    methods:{
        async handleDetailted(){
            this.i= this.$route.params.i;
            let data=await detailtedApi();
            this.list=data.reco_list[this.i];
            console.log(data)
        }
    },
    created(){
        this.handleDetailted();
    }
}
</script>
...
.play > a {
  ...
  padding: 0.033rem 0.033rem;
  border-right: none;
  border-radius: 0.117rem 0 0 0.117rem;
  color: #d00;
}
.image {
  width: 3.125rem;
...
.introduce > p {
  font-size: 0.117rem;
  color: #4d525d;
  margin: 0.042rem 0.083rem 0.033rem 0.083rem;
}
.jianjie {
  font-size: 0.1rem;
...
.introduce > a {
  display: block;
  font-size: 0.092rem;
  margin: -0.033rem 0.083rem 0.083rem 0.083rem;
  color: rgb(255, 70, 60);
...
.new_style-left > u {
  ...
  padding: 0.033rem 0.033rem 0.033rem 0.167rem;
  border-radius: 0.083rem;
  /* background: url(img/8.png) no-repeat 0rem 0.05rem; */
}
.new_style-right {
  clear: both;
  padding-top: 0.05rem;
...
.books_one-p1 {
  ...
  /* background: url(img/9.png) no-repeat 0.267rem; */
}
.books_one-p1 > span {
  display: block;
  float: left;
